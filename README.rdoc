...
* Timesheet::index
* table text size controlled by CSS .small-when-small
* application.rb -> class User def self.has_authority_over

==Next step:
* New User page cancel takes to User's staff page (Users#Show)
* TimesheetHours are sorted by 'created_at DESC' (timesheets_controller), but this could be wrong in theory. Should be sorted by parent timesheet commercial date.
